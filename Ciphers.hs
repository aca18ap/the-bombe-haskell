...
---------------------------------------------------------------------------------------------------------------------------------	
    ...
    type Menu = [Int]
    type IndexedCrib = [(Char, Char, Int)]
    ...
    --LONGEST MENU (not owned)
    {-credit to Emma Norling for the functions below, taken from the lecture slides-}
    menusFrom :: [(Int, Char)] -> IndexedCrib -> [[Int]]
    menusFrom _ [] = [[]]
    menusFrom [] _ = [[]]
    menusFrom ((n, letter):remPos) indexedCrib
        = [n:menu | menu <- menusFrom nextPos filtered]
            ++ menusFrom remPos indexedCrib
        where
        filtered = [(p,c,i) | (p,c,i) <- indexedCrib, i /= n]
        nextPos = [(i, cipher) | (plain, cipher, i) <- filtered, plain == letter]
    
    allMenus :: Crib -> [Menu]
    allMenus (plain, cipher) = menusFrom indexedCipher indexedCrib
        where
        indexedCipher :: [(Int,Char)]
        indexedCipher = zip[0..] cipher
        indexedCrib :: IndexedCrib
        indexedCrib = zip3 plain cipher [0..]
    
    longestMenu :: Crib -> Menu
    longestMenu crib = longest where
        (longest : _) = mergesort size (allMenus crib)
        size a b = length a > length b